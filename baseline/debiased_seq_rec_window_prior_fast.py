...
#%%
# ---------------------------------------------------------------------------
# Fast window-prior negative sampling.
#
# `sample_window_prior_negatives` calls `compute_window_prior` once per
# training event, and each call rescans the entire (m_item, time_len)
# item_time_array from scratch - O(N_events * m_item * time_len) per call.
# It runs once at init and again every `pair_reset_interval` epochs, which
# for realistic m_item (tens of thousands) makes each reset take minutes.
#
# Because item_time_array never changes and every query window is the
# contiguous interval [t - window_size, t), the per-item in-window count
...
# increasing order and maintaining running counts in a Fenwick tree (reused
# from module/sampler.py) turns the O(N * m_item * time_len) scan into
# O((m_item*time_len + N) * log m_item), while producing numerically
# identical sampling probabilities to the original per-event computation.
# ---------------------------------------------------------------------------
