...
from module.sapid_model import SAPID
...
# item interaction counts from training data
item_counts = np.zeros(dataset.m_item, dtype=np.float32)
...
save_prefix = f"_sapid_lr{args.lr}_gamma{args.gamma}_dropout{args.dropout}"
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        anchor_hist = torch.tensor(dataset.train_hist_item_list[hot_sample_idx], dtype=torch.long, device=args.device)

        pos_score = model.residual_score(pos_item, anchor_hist, anchor_user)
        neg_score = model.residual_score(neg_item, anchor_hist, anchor_user)
