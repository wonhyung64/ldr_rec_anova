...
#%%
# This script only varies the Hawkes intensity (mu_v -> mu_v(t)), so unlike
# debiased_seq_rec_hawkes_anova.py there is no --ablation branching here:
# build_time_varying_debias_model is the single wrapper used, matching the
# "none" (shared item embedding) config from the original experiment.
...
debiased_class = build_time_varying_debias_model(model_class)
...
# t_normalized's reference range must come from TRAINING events only (no
# validation/test leakage); see module.debias_time_varying.set_time_normalization.
train_time_min = float(min(dataset.hot_event_time_list.min(), dataset.cold_event_time_list.min()))
train_time_max = float(max(dataset.hot_event_time_list.max(), dataset.cold_event_time_list.max()))
model.set_time_normalization(train_time_min, train_time_max)
...
pattern = f"_{args.model_name}_lambdacen{args.lambda_cen}_tau{args.tau}_scorenorm{args.score_norm}_e???_seed{args.seed}_hawkestimevarying.pt"
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        """CHOICE LOSS: l_choice(Phi, Psi) -- model.prior()'s signature is unchanged,
        so this call is identical to debiased_seq_rec_hawkes_anova.py"""
        ...
        intensity change, since it only uses score_pair (utility), not model.prior()"""
        dataset.get_pair_user_event_timebucket_fast()
    ...
    if epoch % 100 == 0:
        ...
        }, f"{args.save_path}/_{args.model_name}_lambdacen{args.lambda_cen}_tau{args.tau}_scorenorm{args.score_norm}_e{epoch}_seed{args.seed}_hawkestimevarying.pt")
...
if epoch % args.evaluate_interval == 0:
    ...
    for (user, item), pos_time_val in dataset.valid_user_item_time.items():
        ...
        with torch.no_grad():
            ...
            # mu_v(t) now depends on the query time, so (unlike the static-mu
            # model) this must be recomputed per example rather than once for
            # the whole evaluation pass.
            mu, alpha, beta = model.prior_parameters_from_embeddings(pos_time_val)
