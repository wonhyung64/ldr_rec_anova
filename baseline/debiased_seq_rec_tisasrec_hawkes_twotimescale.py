...
#%%
# TiSASRec version of the two-timescale Hawkes prior: only the excitation
# term changes (alpha_v*H_v(t) -> alpha_short_v*H_short_v(t) +
# alpha_long_v*H_long_v(t)), mu_v is unchanged, so -- as with the generic
# sequential two-timescale script -- there is no --ablation branching.
...
pattern = (
    ...
    f"_shl{args.short_half_life}_lhl{args.long_half_life}_e???_seed{args.seed}_hawkestwotimescale.pt"
)
...
while epoch < args.epochs:
    ...
    if epoch % 100 == 0:
        torch.save({
            ...
            f"_shl{args.short_half_life}_lhl{args.long_half_life}_e{epoch}_seed{args.seed}_hawkestwotimescale.pt"
        ))
