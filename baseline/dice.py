...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        # --- Conformity BPR (hot events as "popular" positives,
        #     cold items as less-popular negatives) ---
        # Positive: hot item; negative: cold item sampled at the same batch position
        # When the cold batch is smaller we tile it to match the hot batch size
        if hot_anchor_user.shape[0] > 0 and cold_anchor_user.shape[0] > 0:
            n_hot = hot_anchor_user.shape[0]
            cold_neg_for_con = cold_pos_item[
                np.arange(n_hot) % cold_anchor_user.shape[0]
            ]                                                                    # [n_hot]
            con_pos = model.conformity_score(hot_anchor_user, hot_pos_item)     # [n_hot]
            con_neg = model.conformity_score(hot_anchor_user, cold_neg_for_con) # [n_hot]
    ...
    if epoch % 100 == 0:
        torch.save({
            ...
            "loss": epoch_int_loss,
        }, f"{args.save_path}/dice_lambda{args.lambda1}_lr{args.lr}_alpha{args.alpha}_e{epoch}_seed{args.seed}.pt")
