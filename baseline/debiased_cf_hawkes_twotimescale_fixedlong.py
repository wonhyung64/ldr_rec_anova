...
#%%
# CF (MF) version of the SECOND two-timescale iteration: beta_long is a
# FIXED buffer (not learnable) and alpha_long_mlp's init bias is loosened
# (-6 -> -2), per module.debias_two_timescale_fixedlong. mu_v is unchanged,
# and there is no --ablation branching here either.
...
pattern = (
    ...
    f"_shl{args.short_half_life}_lhl{args.long_half_life}_e???_seed{args.seed}_cfhawkestwotimescalefixedlong.pt"
)
...
while epoch < args.epochs:
    ...
    if epoch % 100 == 0:
        torch.save({
            ...
            f"_shl{args.short_half_life}_lhl{args.long_half_life}_e{epoch}_seed{args.seed}_cfhawkestwotimescalefixedlong.pt"
        ))
