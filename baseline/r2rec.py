...
from module.r2rec_model import R2Rec
...
#%% Compute per-item interaction counts from training data
...
#%% Per-event reweighting: weight[i] = max_count / count(pos_item)
# Precompute weights for each hot training event
hot_item_event_counts = item_counts[dataset.hot_item_list]
max_count = hot_item_event_counts.max()
hot_event_weights = (max_count / hot_item_event_counts.clip(min=1)).astype(np.float32)
...
save_prefix = f"_r2rec_lr{args.lr}_nproto{args.n_proto}_dropout{args.dropout}"
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        # Reweighting: higher weight for tail (low-frequency) items
        sample_weights = torch.tensor(hot_event_weights[hot_sample_idx], device=args.device)  # [B]

        pos_score = model.residual_score(pos_item, anchor_hist_items, anchor_user)  # [B, 1]
        neg_score = model.residual_score(neg_item, anchor_hist_items, anchor_user)  # [B, K]

        bpr = -(F.logsigmoid(pos_score) + F.logsigmoid(-neg_score).sum(-1, keepdim=True))  # [B, 1]
        loss = (sample_weights.unsqueeze(-1) * bpr).mean()
...
if epoch % args.evaluate_interval == 0:
    pred_list, gt_list = [], []
    model.eval()
    model.precompute_item_embeddings()
