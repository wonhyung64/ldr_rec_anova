...
#%%
# This script replaces the (learnable or fixed) two-timescale Hawkes
# excitation with a flexible, signed K-component exponential-basis kernel
# (module.debias_multiscale): lambda_v(t) = softplus(b_v + sum_k w_k(v)*H_k(t)) + eps,
# with FIXED half-lives (spanning a dataset-specific, data-informed log grid
# -- see report_hawkes_multiscale_tune.sh) and UNCONSTRAINED (signed) mixing
# weights w_k(v). mu_v's own parameterization and the choice/centering losses
# are otherwise unchanged, and there is no --ablation branching here either.
half_life_grid = [float(x) for x in args.half_life_grid.split(",")]
...
debiased_class = build_multiscale_debias_model(model_class, half_life_grid)
...
grid_tag = args.half_life_grid.replace(",", "-")
pattern = (
    f"_{args.model_name}_lambdacen{args.lambda_cen}_tau{args.tau}_scorenorm{args.score_norm}"
    f"_hlgrid{grid_tag}_e???_seed{args.seed}_hawkesmultiscale.pt"
)
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        value are unchanged, so this call is identical to the other Hawkes variants"""
    ...
    if wandb_login:
        ...
        # Diagnostics only (see module.debias_multiscale.prior): reuses the
        # last batch's already-built candidate tensors, so this is an extra
        # no-grad forward pass purely for logging, not part of the loss.
        # Reports per-component mean_w_hl{X}/mean_H_hl{X}/mean_contrib_hl{X}
        # for every half-life in the grid, so it's directly visible which
        # timescales the model actually ends up using (and with what sign).
        with torch.no_grad():
            diag = {}
    ...
    if epoch % 100 == 0:
        torch.save({
            ...
            f"_hlgrid{grid_tag}_e{epoch}_seed{args.seed}_hawkesmultiscale.pt"
        ))
...
if epoch % args.evaluate_interval == 0:
    ...
    model.eval()
    with torch.no_grad():
        mu_logit, w, betas = model.prior_parameters_from_embeddings()
    ...
    for (user, item), pos_time_val in dataset.valid_user_item_time.items():
        ...
        for idx2 in range(dataset.m_item // args.batch_size + 1):
            ...
            with torch.no_grad():
                decay = torch.exp(-batch_time_delta.unsqueeze(-1) * betas.view(1, 1, -1))
                H = (decay * batch_time_mask.unsqueeze(-1)).sum(dim=1)
                excitation = (w[item_idx] * H).sum(dim=-1)
                logits = (model.softplus(mu_logit[item_idx] + excitation) + model.eps).flatten()
