...
#%%
# CF (MF) version of the two-timescale Hawkes prior: only the excitation term
# changes (alpha_v*H_v(t) -> alpha_short_v*H_short_v(t) + alpha_long_v*H_long_v(t)),
# mu_v is unchanged, so -- as with the sequential two-timescale script --
# there is no --ablation branching.
...
debiased_class = build_two_timescale_debias_model(
...
pattern = (
    ...
    f"_shl{args.short_half_life}_lhl{args.long_half_life}_e???_seed{args.seed}_cfhawkestwotimescale.pt"
)
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        value are unchanged, so this call is identical to debiased_cf_hawkes_anova.py"""
    ...
    if wandb_login:
        ...
        # Diagnostics only (see module.debias_two_timescale.prior): reuses the
        ...
        with torch.no_grad():
            diag = {}
            model.prior(candidate_items, pos_time, candidate_time_all, diagnostics=diag)
    ...
    if epoch % 100 == 0:
        torch.save({
            ...
            f"_shl{args.short_half_life}_lhl{args.long_half_life}_e{epoch}_seed{args.seed}_cfhawkestwotimescale.pt"
        ))
