...
def compute_window_prior(item_time_array, query_time, window_size, eps=1e-9):
    in_window = (item_time_array > 0) & (item_time_array >= query_time - window_size) & (item_time_array < query_time)
    counts = in_window.sum(axis=1).astype(np.float64) + eps
    return counts / counts.sum()


def sample_window_prior_negatives(item_time_array, event_times, window_size, num_negatives, eps=1e-9):
    negs = np.zeros((len(event_times), num_negatives), dtype=np.int64)
    for i, t in enumerate(event_times):
        prob = compute_window_prior(item_time_array, t, window_size, eps)
...
item_hot_negs = sample_window_prior_negatives(dataset.item_time_array, dataset.hot_event_time_list, window_size, args.contrast_size-1)
item_cold_negs = sample_window_prior_negatives(dataset.item_time_array, dataset.cold_event_time_list, window_size, args.contrast_size-1)
...
while epoch < args.epochs:
    ...
    for idx in range(batch_num):
        ...
        cold_neg_item = torch.tensor(item_cold_negs[cold_sample_idx], dtype=torch.long, device=args.device)
        neg_item = torch.cat([cold_neg_item, hot_neg_item], dim=0)
        ...
        window_logits = np.stack([
            compute_window_prior(dataset.item_time_array, t, window_size)
            for t in event_times
        ])
        logits = torch.tensor(window_logits, dtype=torch.float32, device=args.device)[
            torch.arange(batch_items.shape[0], device=args.device).unsqueeze(1),
            batch_items
        ]
    ...
    if epoch % 100 == 0:
        ...
        },  f"{args.save_path}/{args.model_name}_lambda{args.lambda1}_e{epoch}_seed{args.seed}_window_prior.pt")
...
if epoch % args.evaluate_interval == 0:
    ...
    for (user, item), pos_time_val in dataset.valid_user_item_time.items():
        ...
        window_prob = compute_window_prior(dataset.item_time_array, pos_time_val, window_size)
