...
DATASETS=(kuairand micro_video ml-1m)
BACKBONES=(tisasrec mf grurec sasrec fearec bsarec)
ABLATION=shared
...
for dataset in "${DATASETS[@]}"; do
    for model in "${BACKBONES[@]}"; do
        ...
        for seed in "${SEED_GRID[@]}"; do
            experiments+=("${script} --model-name=${model} --dataset=${dataset} --seed=${seed} --recdim=128 --dropout=0.2 --lr=0.001 --contrast-size=16 --max-seq-len=50 --ablation=${ABLATION} --tau=${TAU} --alpha1=${alpha1} ${extra_args} --score-norm=${SCORE_NORM} --lambda-cen=${gamma} --weights_path=${WEIGHTS_PATH} --evaluate-interval=250 --epochs=500 --debiased_eval=true")
        done
    done
done
